...
export function createDissolve(target, opts = {}) {
  ...
  function applyToMaterial(nodeMat) {
    const { alpha, emissive } = buildNodes();
  ...
  const baseYOffset = baseY0 - _wp.y;
  uBaseY.value = baseY0;
  uHeight.value = height0;
  const cx = (_box.min.x + _box.max.x) / 2, cz = (_box.min.z + _box.max.z) / 2;
  const cxOff = cx - _wp.x, czOff = cz - _wp.z;
  const footR = Math.max((_box.max.x - _box.min.x), (_box.max.z - _box.min.z)) * 0.5 || 0.3;
  ...
  if (cfg.puddle) {
    ...
    const ripple = mx_noise_float(uv().mul(7).add(time.mul(0.25))).mul(0.12).add(0.9);
    const soft = r.oneMinus().smoothstep(0, 0.55).mul(ripple);
    pmat.colorNode = uLiquid.mul(1.15);
    pmat.opacityNode = soft.mul(uPudA);
    puddle = new THREE.Mesh(geo, pmat);
    puddle.renderOrder = 2;
    puddle.position.set(cx, baseY0 + 0.012, cz);
  ...
  function refreshFollow() {
    ...
    if (puddle) puddle.position.set(_wp.x + cxOff, _wp.y + baseYOffset + 0.012, _wp.z + czOff);
  }
  ...
  setProgress(0);
  return { setProgress, update, setParam, dispose, get progress() { return progress; }, puddle };
}
