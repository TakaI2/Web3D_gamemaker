...
import {
  attribute, uv, texture, uniform, vec2, vec3, vec4, float, mod, floor, PI, select, cameraViewMatrix,
} from 'https://esm.sh/three@0.184.0/tsl';
import * as PTSL from 'https://esm.sh/three@0.184.0/tsl';
const atan2p = PTSL.atan2 || PTSL.atan;   // 2引数 atan の互換
...
function randomConeDir(dir, spreadRad, out) {
  ...
  // dir と out が同一オブジェクトでも壊れないよう、dir 成分を退避してから書き込む
  const dx = dir.x, dy = dir.y, dz = dir.z;
  out.copy(t1).multiplyScalar(Math.cos(phi) * r)
    .addScaledVector(t2, Math.sin(phi) * r);
  out.x += dx * z; out.y += dy * z; out.z += dz * z;
...
export function createFxSystem(initial) {
  ...
  // 発生原点オフセット（ワールド空間トレイル用）。既定0。setEmitOrigin で移動すると、
  // 既に生成済みの粒子はその場に残り、新規粒子だけが新原点で生まれる＝軌跡(トレイル)になる。
  // ※このモードでは object3D は原点(identity)に置き、px を実質ワールド座標として扱う。
  const emitOrigin = new THREE.Vector3(0, 0, 0);
  ...
  let iPosAttr, iSizeAttr, iColAttr, iFrameAttr, iRotAttr, iVelAttr;
  function bindInstanced() {
    ...
    iVelAttr = new THREE.InstancedBufferAttribute(vel, 3).setUsage(THREE.DynamicDrawUsage);   // 速度（ストレッチ用）
    ...
    geom.setAttribute('iVel', iVelAttr);
  ...
  // 速度ストレッチ：粒を速度方向へ引き伸ばす（しずく/流れ＝液体感）。stretch=0 で従来どおり。
  const uStretch = uniform(cfg.stretch ?? 0);
  const viewVel = cameraViewMatrix.mul(vec4(iVel, 0)).xy;   // ビュー平面（画面）上の速度
  const speed = viewVel.length();
  const stretchFac = float(1).add(uStretch.mul(speed)).min(6);          // 縦方向の伸び（速いほど長く・上限6倍）
  const velAngle = atan2p(viewVel.y, viewVel.x).sub(PI.mul(0.5));        // 局所+Yを速度方向へ向ける
  const useStretch = uStretch.greaterThan(0.0001);
  const scaleStretch = vec2(iSize, iSize.mul(stretchFac));
  ...
  material.scaleNode = select(useStretch, scaleStretch, vec2(iSize, iSize));   // ストレッチ時は速度方向へ伸縮
  material.rotationNode = select(useStretch, velAngle, iRot);                  // ストレッチ時は速度方向へ整列
  ...
  function spawn(i, t) {
    ...
    px[i * 3] = tmpV.x + emitOrigin.x; px[i * 3 + 1] = tmpV.y + emitOrigin.y; px[i * 3 + 2] = tmpV.z + emitOrigin.z;
  ...
  /** 発生原点（ワールド空間）をセット。トレイル用：object3D は原点に置き、これを毎フレーム移動する。 */
  function setEmitOrigin(x, y, z) { emitOrigin.set(x, y, z); }
  ...
  function update(dt) {
    ...
    // 床はワールドYで指定。自分(object3D)のワールドY分を差し引いてローカル閾値にする
    // → エフェクトを持ち上げても「地面(ワールドY=floorY)」まで落ちてから弾ける。
    const floorOn = cfg.floorMode && cfg.floorMode !== 'none';
    let floorLocal = -Infinity;
    if (floorOn) { mesh.updateWorldMatrix(true, false); floorLocal = (cfg.floorY ?? 0) - mesh.matrixWorld.elements[13]; }
    for (let i = 0; i < n; i++) {
      ...
      // 床コリジョン。bounce=反発係数で弾けて横へ広がる＝水のはじけ。
      if (floorOn && px[i * 3 + 1] < floorLocal) {
        if (cfg.floorMode === 'kill') { alive[i] = 0; aCol[i * 4 + 3] = 0; continue; }
        px[i * 3 + 1] = floorLocal;
        if (vel[i * 3 + 1] < 0) vel[i * 3 + 1] = -vel[i * 3 + 1] * (cfg.bounce ?? 0.4);
        vel[i * 3] *= 0.55; vel[i * 3 + 2] *= 0.55;   // 接地摩擦（横へ広がりつつ減速）
      }
      aRot[i] += spinV[i] * dt;
      writeParticle(i);
    }
  ...
  return { object3D: mesh, update, burst, setEmitting, setEmitOrigin, setConfig, getConfig, dispose };
}
