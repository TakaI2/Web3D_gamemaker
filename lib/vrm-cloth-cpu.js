...
export function createVRMClothCPU(o) {
  ...
  function makeSideMat(side, colorHex, dir) {
    const mat = new THREE.MeshPhysicalMaterial({
      ...
      // 黒が保存されている個体があるため正規化（黒＝光沢が効かない）
      sheenColor: new THREE.Color(normalizeSheenColor(m.sheenColor)),
  ...
  return {
    ...
    get gripInfo() { return gripGroups.map((g) => ({ id: g.id, bone: g.bone, active: g.active })); },
  };
}
