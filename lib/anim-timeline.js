...
export function createAnimTimeline({ els, getVrm, getHipsRestY, setStatus = () => {}, onPoseEdited = () => {} }) {
  const tl = { keys: [], t: 0, dur: 4, playing: false, loop: true, selKey: -1, bs: new Map(), selBs: null };
    ...
    rows.forEach((row, ri) => {
      ...
      if (row === '__pose__') {
        tl.keys.forEach((k, i) => {
          ...
          if (x > HEADER_W - 8 && x < W + 8) drawKeyDiamond(ctx, x, yc, i === tl.selKey && !tl.selBs, '#ffd060');
        });
      } else {
        (tl.bs.get(row) || []).forEach((k, i) => {
          const x = f2x(Math.round(k.t * FPS));
          if (x > HEADER_W - 8 && x < W + 8) drawKeyDiamond(ctx, x, yc, tl.selBs?.name === row && tl.selBs?.i === i, '#5fd0ff');
        });
      }
    });
