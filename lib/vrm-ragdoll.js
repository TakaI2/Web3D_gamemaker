...
// 自己衝突の関節半径（m）。胴体を太く、四肢を細く＝腕脚が胴体表面で止まる。bodyScale で一律スケール。
const COLLISION_RADIUS = {
  hips: 0.12, spine: 0.12, chest: 0.13, upperChest: 0.12, neck: 0.05, head: 0.09,
  leftShoulder: 0.06, rightShoulder: 0.06, leftUpperArm: 0.05, rightUpperArm: 0.05,
  leftLowerArm: 0.045, rightLowerArm: 0.045, leftHand: 0.04, rightHand: 0.04,
  leftUpperLeg: 0.08, rightUpperLeg: 0.08, leftLowerLeg: 0.06, rightLowerLeg: 0.06,
...
const DEFAULTS = {
  ...
  limitDamp:     0.4,    // 角度制限で捕まった粒子の速度減衰(0..1/フレーム)。重力起因の速度を抜き、制限位置でのバズ振動を防ぐ
  selfCollision: true,   // 自己衝突：関節を球とみなし四肢が胴体へ埋まるのを防ぐ
  bodyScale:     1.0,    // 自己衝突半径の倍率（体の太さ）。大きいほど四肢が外へ
  rigidBones:    [],     // ここに挙げたボーンはラグドール化しない（剛体＝親のレスト姿勢に追従）。例: 肩・脚の付け根
...
export function createRagdoll(vrm, opts = {}) {
  ...
  // rigidBones に挙げたボーンはラグドール化しない（剛体扱い＝粒子を作らず親のレスト姿勢に追従）。
  const rigid = new Set(o.rigidBones || []);
  const tracked = TRACKED.filter((b) => !rigid.has(b));
  ...
  // 自己衝突ペア：骨で繋がっていない（距離拘束・曲げ拘束いずれにも無い）非近傍ペアを列挙。
  const pairKey = (i, j) => (i < j ? `${i}_${j}` : `${j}_${i}`);
  const connected = new Set();
  for (const c of constraints) connected.add(pairKey(c.i, c.j));
  for (const c of bendConstraints) connected.add(pairKey(c.i, c.j));
  const selfPairs = [];
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      if (connected.has(pairKey(i, j))) continue;
      const ri = COLLISION_RADIUS[particles[i].bone] ?? 0.05;
      const rj = COLLISION_RADIUS[particles[j].bone] ?? 0.05;
      selfPairs.push({ i, j, min: ri + rj });
    }
  }

  return {
    vrm, opts: o, particles, constraints, bendConstraints, angleLimits, selfPairs, bones, idxOf,
    _limited: new Uint8Array(particles.length),   // そのフレームで角度制限に捕まった粒子の印
...
export function updateRagdoll(rd, dt, env = {}) {
  ...
  const dt2 = dt * dt;
  const keep = 1 - o.drag;
  if (rd._limited) rd._limited.fill(0);   // フレーム頭でクリア
  ...
  for (let it = 0; it < o.iterations; it++) {
    ...
    // 自己衝突：骨で繋がっていない関節ペアを球として押し出し、四肢が胴体へ埋まるのを防ぐ
    if (o.selfCollision) {
      for (const s of rd.selfPairs) {
        const a = rd.particles[s.i].pos, b = rd.particles[s.j].pos;
        _d.copy(b).sub(a);
        const dist = _d.length() || 1e-6;
        const min = s.min * o.bodyScale;
        if (dist < min) {
          _d.multiplyScalar((dist - min) / dist * 0.5);
          a.add(_d); b.sub(_d);
        }
      }
    }
    ...
    for (const L of rd.angleLimits) {
      ...
      if (target !== beta) {
        ...
        if (rd._limited) rd._limited[L.c] = 1;   // この粒子は制限に捕まった
      }
    }
    for (const p of rd.particles) {
  ...
  // 角度制限で捕まった粒子は速度（prev→pos）を抜く。重力が毎フレーム入れる速度の
  // 汲み上げを断ち、満たせない姿勢（頭ピン等）での永久バズ振動を抑える。
  if (o.limitDamp > 0 && rd._limited) {
    for (let i = 0; i < rd.particles.length; i++) {
      if (rd._limited[i]) { const p = rd.particles[i]; p.prev.lerp(p.pos, o.limitDamp); }
    }
  }
