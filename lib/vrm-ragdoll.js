...
export function createRagdoll(vrm, opts = {}) {
  ...
  for (const d of bones) {
    ...
    angleLimits.push({ a: ai, b: bi, c: ci, restAngle, maxBend });
  }
...
export function setRagdollActive(rd, active) {
  ...
  if (active) {
    ...
    // レスト（バインド）姿勢へ戻す。呼び出し側がアニメを再開すれば上書きされる。
    for (const d of rd.bones) {
      d.node.quaternion.copy(d.restLocalQuat);
      d.node.position.copy(d.restLocalPos);
    }
    // カリング設定を復元し、表示を戻す
...
export function disposeRagdoll(rd) {
  if (!rd) return;
  if (rd.active) setRagdollActive(rd, false);
