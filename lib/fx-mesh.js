...
function buildMeshLayer(layer) {
  ...
  // テクスチャ角度：UVを中心まわりに回転（円柱などでスプライトの貼付角度を変える）
  const angRad = (layer.texAngle || 0) * Math.PI / 180;
  let bUv = uv();
  if (angRad) {
    const c = Math.cos(angRad), s = Math.sin(angRad);
    const cu = uv().x.sub(0.5), cv = uv().y.sub(0.5);
    bUv = vec2(cu.mul(c).sub(cv.mul(s)).add(0.5), cu.mul(s).add(cv.mul(c)).add(0.5));
  }
  ...
  if (fr && (fr.cols || 1) * (fr.rows || 1) > 1) {
    ...
    uvNode = vec2(
      fcol.add(bUv.x).div(cols),
      float(1).sub(frow.add(float(1).sub(bUv.y)).div(rows)),   // frame0=左上
    );
  } else {
    uvNode = bUv.mul(vec2(repeat[0], repeat[1])).add(vec2(scroll[0], scroll[1]).mul(time));
  }
...
function buildParticleLayer(layer) {
  ...
  if (Array.isArray(layer.lifetime)) cfg.lifetime = layer.lifetime.slice();   // 寿命[min,max]
  if (layer.maxParticles != null) cfg.maxParticles = layer.maxParticles;
  if (layer.blending) cfg.blending = layer.blending;
  if (layer.texture) cfg.texture = fxTextureSrc(layer.texture);
  // 重力（Y加速度。負=落下/正=上昇）と抵抗。未指定はプリセット既定。
  if (layer.gravity != null) cfg.acceleration = [cfg.acceleration[0], layer.gravity, cfg.acceleration[2]];
  if (layer.drag != null) cfg.drag = layer.drag;
  if (layer.stretch != null) cfg.stretch = layer.stretch;   // 速度方向ストレッチ（液体の流れ/しずく）
  if (layer.floorMode) cfg.floorMode = layer.floorMode;     // 床コリジョン none/bounce/kill
  if (layer.floorY != null) cfg.floorY = layer.floorY;
  if (layer.bounce != null) cfg.bounce = layer.bounce;
  // エミッタ形状（point/disc=平面/box）・発生方向(emitter.rotation)・拡散(spreadDeg)・速度
  const em = cfg.emitter, ve = cfg.velocity;
  cfg.emitter = {
    shape: layer.shape || em.shape,
    radius: layer.radius != null ? layer.radius : em.radius,
    size: layer.boxSize || em.size,
    rotation: layer.dir || em.rotation,
  };
  cfg.velocity = {
    speed: layer.speed || ve.speed,
    spreadDeg: layer.spread != null ? layer.spread : ve.spreadDeg,
  };
  // 回転：初期角度(startDeg)/回転速度(speedDeg) 各 [min,max]。未指定はプリセット既定。
  if (layer.spinStart || layer.spinSpeed) {
    if (!cfg.spin) cfg.spin = { startDeg: [0, 0], speedDeg: [0, 0] };
    if (layer.spinStart) cfg.spin.startDeg = layer.spinStart;
    if (layer.spinSpeed) cfg.spin.speedDeg = layer.spinSpeed;
  }
  // スプライトシート（タイル）アニメ。mode: overLife(寿命で1巡)/loop/once
  if (layer.frames && (layer.frames.cols || 1) * (layer.frames.rows || 1) > 1) {
    cfg.frames = { cols: layer.frames.cols || 1, rows: layer.frames.rows || 1, fps: layer.frames.fps || 12, mode: layer.frames.mode || 'overLife' };
  }
...
export function createMeshFx(spec) {
  ...
  // ワールド空間トレイル用：粒子レイヤーの発生原点を移動（メッシュ/トルネードは追従しない）。
  // 呼び出し側は group(object3D) を原点に置くこと。
  function setEmitOrigin(x, y, z) { for (const pl of particles) pl.fx.setEmitOrigin(x, y, z); }
  function dispose() {
  ...
  return { object3D: group, update, setEmitting, burst, setEmitOrigin, dispose };
}
