// lib/terrain.js — 編集可能ハイトマップ地形（map-editor と city-fly で共用）。
...
export function noiseTerrain(data, { seed = 1, amp = 260, freq = 4, octaves = 4, flatten = 0.35 } = {}) {
  ...
  for (let j = 0; j < res; j++) {
    for (let i = 0; i < res; i++) {
      ...
      h /= norm;
      h = Math.max(0, h - flatten) / (1 - flatten);   // 低地を平らに（街を置ける場所を作る）
...
export function buildRoadGraph(roads, { step = 20 } = {}) {
  ...
  const link = (a, b, kind) => {
    ...
    seen.add(ek);
    edges.push([a, b, kind]);   // kind=元スプラインの種別（avenue等。未指定はundefined＝従来通り）
  ...
  for (const r of roads || []) {
    ...
    for (const p of pts) {
      ...
      if (prev) link(prev, k, r.kind);
      prev = k;
    }
    if (r.closed && pts.length > 2) link(prev, key(pts[0].x, pts[0].z), r.kind);
  }
  return { nodes, edges };
}
