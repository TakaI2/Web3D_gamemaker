...
export function createVRMCloth(o) {
  ...
  const frameShiftUniform      = uniform(new THREE.Vector3());   // フレーム剛体シフト（高速移動の超過分を布に感じさせない）

  // ── 力学コンピュート ──
  // フレーム剛体シフト: 全頂点を平行移動（速度=forceには触れない＝布は移動を感じない）。
  // アンカー頂点は次のcomputeVertexForcesでボーン位置へ吸着し直されるため全頂点でよい
  const computeFrameShift = Fn(() => {
    vertexPositionBuffer.element(instanceIndex).addAssign(frameShiftUniform);
  })().compute(vertexCount).setName('Cloth_Shift');
  ...
  function _update(dt, frame) {
    ...
    if (steps > 0) {   // 蓄積された剛体シフトをサブステップ前に一括適用（steps=0のフレームは持ち越し）
      const fs = frameShiftUniform.value;
      if (fs.x !== 0 || fs.y !== 0 || fs.z !== 0) {
        renderer.compute(computeFrameShift);
        fs.set(0, 0, 0);
      }
    }
  ...
  return {
    ...
    addFrameShift: (x, y, z) => { const v = frameShiftUniform.value; v.x += x; v.y += y; v.z += z; },   // このフレームの剛体移動量（布に感じさせない分）
