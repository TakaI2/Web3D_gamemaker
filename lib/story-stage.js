...
export async function createStoryStage({ container, mode = 'play' }) {
  ...
  // 「クリック/Space で送り」のヒント表示
  const hintEl = document.createElement('div');
  hintEl.textContent = '▼ クリック / Space';
  hintEl.style.cssText = 'position:absolute;left:50%;bottom:10px;transform:translateX(-50%);color:#fff;font:12px system-ui;background:rgba(0,0,0,0.55);padding:3px 12px;border-radius:12px;z-index:55;display:none;pointer-events:none;';
  container.appendChild(hintEl);

  let advanceResolve = null;
  function waitAdvance() { hintEl.style.display = 'block'; return new Promise(r => { advanceResolve = r; }); }
  function doAdvance() { hintEl.style.display = 'none'; if (advanceResolve) { const r = advanceResolve; advanceResolve = null; r(); } }
  // クリックは container 全体で受ける（プレビュー領域のどこをクリックしても送れる）
  container.addEventListener('click', doAdvance);
  ...
  function dispose() {
    ...
    container.removeEventListener('click', doAdvance);
    hintEl.remove();
    actorManager.clear();
    stopBgm();
    renderer.dispose();
