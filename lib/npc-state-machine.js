...
const DEFAULT_BEHAVIOR = {
  ...
  recoverDelaySec: 2.5,
  tauntChance:   0.12, // idle から挑発へ移る毎秒確率（0で無効）
  tauntRangeMin: 5,    // 挑発時に寄る距離の最小(m)
  tauntRangeMax: 10,   // 挑発時に寄る距離の最大(m)（毎回この範囲でランダム）
};

// 状態既定値。lookAtEye/Head は 0-1（視線=目 / 顔=頭 のプレイヤー追従強度）。
const DEFAULT_STATES = {
  idle:       { expression: {},                 lookAtEye: 1.0, lookAtHead: 0.35, durationSec: 0 },
  ...
  taunt:      { expression: { happy: 0.6 },     lookAtEye: 1.0, lookAtHead: 0.85, durationSec: 4.0 },
...
function update(sm, dt, ctx) {
  ...
  if (ctx.ragdollActive) {
    ...
    if (sm.state === 'idle') {
      if (b.tauntChance > 0 && Math.random() < b.tauntChance * dt) {
        sm.state = 'taunt';
        sm.timer = (sm.states.taunt && sm.states.taunt.durationSec) || 4;
        const lo = b.tauntRangeMin ?? 5, hi = b.tauntRangeMax ?? 10;
        sm.tauntDist = lo + Math.random() * Math.max(0, hi - lo);   // 5〜10m など範囲でランダム
      } else if (b.sightRange > 0 && dist < b.sightRange && Math.random() < b.detectChance * dt) {
        sm.state = 'alert';
        sm.timer = sm.states.alert.durationSec || 0.6;
      }
    } else if (sm.state === 'taunt') {
      sm.timer -= dt;
      if (sm.timer <= 0) { sm.state = 'idle'; sm.timer = 0; }
