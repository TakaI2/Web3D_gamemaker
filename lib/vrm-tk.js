...
export function createTkBeam(spec = {}) {
  ...
  const mids = [new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3()];
  let midT = 0, lastW = -1;
  return {
    objects: beams.map((b) => b.object3D),
    show(from, to, dt, camPos) {
      ...
      if (midT <= 0) {   // 中間点3つを短周期で振り直し＝電撃の波打ち
        ...
        for (let i = 0; i < 3; i++) {
          mids[i].lerpVectors(from, to, (i + 1) / 4);
          _r.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5);
          _r.addScaledVector(_d, -_d.dot(_r));
          if (_r.lengthSq() > 1e-6) mids[i].addScaledVector(_r.normalize(), dist * (0.05 + Math.random() * 0.1));
        }
      }
      // 距離に応じて幅も同縮尺（短いビームで電撃模様が圧縮されないように）
      const w = baseWidth * Math.min(1.6, Math.max(0.12, dist / refLen));
      if (Math.abs(w - lastW) > 0.003) {
        lastW = w;
        for (const b of beams) b.setParam?.('width', w);
      }
      const pts = [from, mids[0], mids[1], mids[2], to];
      for (let i = 0; i < 4; i++) {
        beams[i].object3D.visible = true;
        beams[i].setEndpoints(pts[i], pts[i + 1], camPos);
      }
    },
    hide() { for (const b of beams) b.object3D.visible = false; },
    dispose() { for (const b of beams) b.dispose?.(); },
  };
}
