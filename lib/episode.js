...
// startWhen: タイトルの「ゲームスタート」を解禁する条件。
//   'world' = ステージ構築（パイプラインのコンパイル含む）まで待つ。OP再生は滑らか。
//   'cast'  = 会話キャストが揃えば開始し、ステージはOPの裏で構築する。開始は早いが、
//             ステージのコンパイルがOP中に走ると1〜2秒のコマ落ちになる（実測値は .tmp/design.md）。
export const STAGE_RULES = {
  // titleSleep: タイトルを「GIF背景＋眠るネイ」にする（本編でも使えるようエピソード側で指定できる）
  rooms: { wanted: false, buildingEntry: false, cars: false, agents: false, special: false, paramsHud: false, fixedHour: 12, startWhen: 'world', titleSleep: true },
  city:  { wanted: true,  buildingEntry: true,  cars: true,  agents: true,  special: true,  paramsHud: true,  fixedHour: null, startWhen: 'world', titleSleep: false },
};
