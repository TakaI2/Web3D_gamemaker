...
const EXPR_RESET = ['neutral', 'happy', 'angry', 'sad', 'relaxed', 'surprised'];
...
export function createScenario2DStage(opts = {}) {
  ...
  function begin(actorId, face, text, extra) {
    ...
    if (em) {   // 表情: 行の expression（VRM表情名）＞ face（2D表情名→VRM表情へ変換）
      for (const nm of EXPR_RESET) { try { em.setValue(nm, 0); } catch { /* noop */ } }
