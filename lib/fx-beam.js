...
export function createBeamFx(spec = {}) {
  const geoW = 0.4;                    // ジオメトリの基準幅（実際の幅は帯メッシュのYスケールで可変）
  let width = spec.width ?? geoW;      // 帯の見かけ幅(m)。setParam('width') でライブ変更可
  ...
  const uFreq     = uniform(spec.freq ?? 15);       // ギザギザの細かさ（10mあたり）
  const uScroll   = uniform(spec.scroll ?? 0.6);    // テクスチャの流れ
  const uRepeat   = uniform(spec.repeat ?? 3);      // 長さ方向のタイル密度（10mあたり）
  // 距離正規化: 密度系を「10mあたり」に換算。
  // uLenN   = ギザギザ(頂点揺れ)用。上限60m＝超遠距離で波長が細かくなりすぎて毛羽立つのを防ぐ
  // uLenTile= テクスチャタイル用。上限なし＝何mでも模様の実寸が一定（引き伸ばさない＝遠距離でも細く見えない）
  const uLenN = uniform(1);
  const uLenTile = uniform(1);
  const uSheetTiles = uniform(1);   // sheetスタイルのタイル数（整数＝端で模様が切れない）
  ...
  let sheetTileM = spec.sheetTile ?? 12;   // sheetスタイル: タイル1枚の実寸(m)
  const geo = new THREE.PlaneGeometry(1, geoW, 64, 1);
  ...
  let beamTex;
  if (style === 'sheet') {
    // シート連続タイル：1コマを全長に引き伸ばさず、一定実寸（既定12m）のタイルで繰り返す。
    // タイルごとにコマを1つずらす＝同じ模様の並びに見えない（横揺れ無し＝まっすぐ）
    const aTileS = uv().x.mul(uSheetTiles);
    const tileI = floor(aTileS);
    const fiS = mod(floor(time.mul(uFps)).add(tileI), uTotal);
    const fcolS = mod(fiS, uCols);
    const frowS = floor(fiS.div(uCols));
    const cellU = fcolS.add(fract(aTileS)).div(uCols);
    const cellV = float(1).sub(frowS.add(float(1).sub(uv().y)).div(uRows));
    ...
    const freqL = uFreq.mul(uLenN);   // 10mあたり密度 × 現在長 ＝ 距離に依らず同じ見た目
    const wob = sin(along.mul(freqL).add(time.mul(9)))
      .add(sin(along.mul(freqL.mul(2.3)).add(time.mul(15))).mul(0.5))
      .add(sin(along.mul(freqL.mul(4.7)).add(time.mul(23))).mul(0.25));
    ...
    const uAlong = uv().x.mul(uRepeat.mul(uLenTile)).add(time.mul(uScroll));
  ...
  mesh.scale.set(1, width / geoW, 1);   // 帯幅を初期反映
  ...
  const alongF = rotV.mul(uTubeRepeat.mul(uLenTile)).add(time.mul(uTubeScroll));   // 長さ方向＝流れ（10mあたり密度・実寸一定）
  ...
  function setEndpoints(from, to, camPos) {
    ...
    // 距離正規化（10m基準）。ギザギザだけ60m上限（毛羽立ち防止）、テクスチャは上限なし＝実寸一定
    uLenN.value = Math.max(0.1, Math.min(len, 60) / 10);
    uLenTile.value = Math.max(0.1, len / 10);
    uSheetTiles.value = Math.max(1, Math.round(len / sheetTileM));
  ...
  function setParam(key, val) {
    if (key === 'width') { width = val; mesh.scale.set(1, val / geoW, 1); }   // 帯幅（経路リボンにも反映）
    else if (key === 'color') uColor.value.set(val);
    ...
    else if (key === 'sheetTile') sheetTileM = Math.max(2, val);
