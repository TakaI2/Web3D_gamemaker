...
//   'expr'(表情プリセット) | 'select'(options 指定) | 'combo'(options 候補つき自由入力)
...
export const STORY_OPS = {
  'say':             { label: 'セリフ',       blocking: true, fields: [
    ...
    { key: 'lipCps', type: 'number', def: 9 },   // 口パク速度（文字/秒）。3Dステージ表示時のリップシンクに使用
    { key: 'face',   type: 'combo',  def: '',    // 2D紙芝居用: 顔グラ表情名（scenario2d/face/<actor>/<face>.png）。3Dステージではスマイル等をVRM表情へ変換
      options: ['normal', 'smile', 'angry', 'worry', 'panic', 'weak', 'damage'] },
  ] },
  'wait':            { label: 'クリック待ち', blocking: true, fields: [] },
  'actor.show':      { label: '登場',         blocking: true, fields: [
  ...
  'stage':           { label: 'ステージ',     blocking: true, fields: [
    { key: 'name', type: 'stageRef', def: 'stage.json' },
  ] },
  'bg':              { label: '背景',         fields: [
    { key: 'color', type: 'text', def: '#bcd8ef' },
    { key: 'image', type: 'bgRef', def: '' },   // 2D紙芝居用: 背景（scenario2d/bg/… または 'gif/xxx.gif' の public 直下参照）
  ] },
  'bgm.play':        { label: 'BGM再生',      fields: [
    { key: 'name',   type: 'bgmRef' },   // 'BGM/xxx.ogg'（public直下参照）。素の名前なら sound/ 配下
