...
//   say{actor,lines,cps,face,lipCps} … メッセージウィンドウ＋顔グラ（face=表情。2D拡張）
//     lines の各要素は文字列 or {text,expression,weight}（expression=VRM表情名。3Dステージ表示時に使用）
//     lipCps=口パク速度（文字/秒）。face/expression/lipCps は stage.begin の第4引数 extra へ渡す
...
export function createScenario2D(opts = {}) {
  ...
  const rootPath = opts.rootPath || '../';   // public直下（'/'入りの name/image 用。distでは './' が渡る）
  // スマホは画面が狭いので、ウィンドウを下端に密着させた細身レイアウトにする（PCは従来どおり）
  const MOBILE = (() => { try { return window.matchMedia?.('(pointer: coarse)').matches || 'ontouchstart' in window; } catch { return false; } })();
  const FACE_PX = MOBILE ? 76 : 144, TYPE_CPS_DEF = 18;
  ...
  function ensureDom() {
    ...
    win.style.cssText = (MOBILE
      ? 'position:absolute;left:0;right:0;bottom:0;width:100%;border-radius:10px 10px 0 0;padding:7px 10px;gap:8px;'
      : 'position:absolute;left:50%;bottom:42px;transform:translateX(-50%);width:min(1150px,94vw);border-radius:14px;padding:16px 22px;gap:14px;')
      + 'background:rgba(8,10,24,0.86);border:1px solid rgba(140,150,255,0.5);'
      + 'box-shadow:0 4px 20px rgba(0,0,0,0.6);display:none;align-items:center;';
    const face = document.createElement('div');
    face.style.cssText = `width:${FACE_PX}px;height:${FACE_PX}px;flex:0 0 ${FACE_PX}px;border-radius:${MOBILE ? 6 : 8}px;overflow:hidden;position:relative;background:#223;`;
    const faceFb = document.createElement('div');
    faceFb.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font:900 '
      + (MOBILE ? 38 : 68) + 'px Meiryo,sans-serif;color:#fff;';
    ...
    name.style.cssText = MOBILE ? 'font:700 13px Meiryo,sans-serif;margin-bottom:2px;' : 'font:700 21px Meiryo,sans-serif;margin-bottom:7px;';
    const text = document.createElement('div');
    text.style.cssText = (MOBILE ? 'font:15px/1.45 Meiryo,sans-serif;min-height:2.9em;' : 'font:25px/1.7 Meiryo,sans-serif;min-height:3.4em;')
      + 'color:#eef;white-space:pre-wrap;';
    const cue = document.createElement('div');
    cue.style.cssText = 'position:absolute;right:' + (MOBILE ? 8 : 14) + 'px;bottom:' + (MOBILE ? 2 : 8)
      + 'px;color:#9ab;font:' + (MOBILE ? 10 : 12) + 'px Meiryo,sans-serif;opacity:0;transition:opacity 0.3s;';
    cue.textContent = '▼';
    body.appendChild(name); body.appendChild(text);
    win.appendChild(face); win.appendChild(body); win.appendChild(cue);
    const hint = document.createElement('div');
    hint.style.cssText = 'position:absolute;right:' + (MOBILE ? 8 : 14) + 'px;top:' + (MOBILE ? 6 : 10)
      + 'px;color:#778;font:' + (MOBILE ? 10 : 12) + 'px Meiryo,sans-serif;';
    hint.textContent = MOBILE ? 'タップ: 次へ' : 'クリック: 次へ ／ Esc: スキップ';
    ...
    els = { root, bg, bgImg, win, face: els_face_ref, faceFb, faceImg, name, text, cue, fade };
  }

  function play(story, o = {}) {
    ensureDom();
    st.active = true; st.story = story; st.idx = 0; st.onEnd = o.onEnd || null;
    st.line = null; st.lineQ = []; st.mode = null; st.fadeDir = 0; st.onStage = false;
  ...
  function finish() {
    st.active = false;
    if (opts.stage && opts.stage.end) opts.stage.end();
    if (els) { els.root.style.display = 'none'; els.root.style.background = '#000'; els.bg.style.display = ''; els.face.style.display = ''; }
    stopBgm(400);
  ...
  function showLine(q) {   // q = {actor,face,text,cps,lipCps,expression,weight}
    const actorId = q.actor, face = q.face, line = q.text, cps = q.cps;
    const a = actorOf(actorId);
    // ステージ表示（3Dで話者を大きく映す）。有効なら背景と顔枠を消して 3D を透かす
    const onStage = !!(opts.stage && opts.stage.begin
      && opts.stage.begin(actorId, face, line, { lipCps: q.lipCps, expression: q.expression, weight: q.weight }));
    st.onStage = onStage;   // bgImg.onload（非同期）がステージ表示を覆わないための状態
    els.root.style.background = onStage ? 'transparent' : '#000';
    els.bg.style.display = onStage ? 'none' : '';
    els.bgImg.style.display = onStage ? 'none' : els.bgImg.style.display;
    els.face.style.display = onStage ? 'none' : '';
  ...
  function step() {   // 次のblocking opまで実行
    while (st.active) {
      ...
      if (st.lineQ.length) { showLine(st.lineQ.shift()); return; }
      if (st.idx >= sc.length) { finish(); return; }
      const op = sc[st.idx++];
      const kind = op.op;
      if (kind === 'say') {
        const lines = Array.isArray(op.lines) ? op.lines : [String(op.lines || '')];
        for (const t of lines) {   // 行は文字列 or {text,expression,weight}
          const o = t && typeof t === 'object' ? t : null;
          st.lineQ.push({ actor: op.actor, face: op.face, text: o ? (o.text || '') : String(t ?? ''),
            cps: op.cps ? op.cps * 2.2 : 0,   // エディタcps(遅め)→2D体感に補正
            lipCps: op.lipCps, expression: o ? o.expression : undefined, weight: o ? o.weight : undefined });
        }
      } else if (kind === 'bg') {
        if (op.color) els.bg.style.background = /^#/.test(op.color) ? `linear-gradient(180deg,${op.color},#000)` : op.color;
        els.bgImg.style.display = 'none';
        if (op.image) {
          els.bgImg.onload = () => { els.bgImg.style.display = st.onStage ? 'none' : ''; };   // ステージ(3D)表示中はDOM背景で覆わない
          els.bgImg.onerror = () => { els.bgImg.style.display = 'none'; };   // 仮=グラデ背景のまま
          els.bgImg.src = op.image.includes('/') ? (rootPath + op.image) : (basePath + '/bg/' + op.image);   // '/'入り＝public直下参照（例: gif/xxx.gif）
        }
        if (opts.stage && opts.stage.bg) opts.stage.bg(op.image || null);   // 3Dステージ側の背景（画像/GIF）
      } else if (kind === 'bgm.play') {
        stopBgm(0);
        try {
          const src = op.name.includes('/') ? (rootPath + op.name) : (soundPath + '/' + op.name);   // '/'入り＝public直下参照（例: BGM/xxx.ogg）
          bgm = new Audio(src); bgm.loop = op.loop !== false; bgm.volume = op.volume ?? 0.6; bgm.play().catch(() => { /* 自動再生制限 */ });
        } catch { /* noop */ }
      } else if (kind === 'bgm.stop') {
        stopBgm(op.fade ?? 500);
      } else if (kind === 'se') {
        try { const a = new Audio(op.name.includes('/') ? (rootPath + op.name) : (soundPath + '/' + op.name)); a.volume = op.volume ?? 1; a.play().catch(() => { /* noop */ }); } catch { /* noop */ }
      } else if (kind === 'delay') {
