...
import { buildLifeData, findPath, scheduleAt } from '../lib/room-life.js';
...
const ROOM_SCALE = 1.5;      // 表示スケール（家具キットはVRM比で小さい。ゲーム内装のINTERIOR_SCALEと同値）
let TILE0 = 1;               // floorFull の一辺（実測・未スケール。fp計算や生活グリッドはこちら）
let TILE = 1;                // 表示用の一辺 = TILE0 × ROOM_SCALE
...
async function spawnItem(parent, item, selectable) {
  ...
  if (item.stackOn) { try { y += (await getModel(item.stackOn)).size.y * ROOM_SCALE; } catch { /* 台なし */ } }
  g.scale.setScalar(ROOM_SCALE);
...
async function saveUnit() {
  ...
  const fp = [Math.max(1, Math.ceil((bb.max.x - bb.min.x) / TILE0 - 0.15)), Math.max(1, Math.ceil((bb.max.z - bb.min.z) / TILE0 - 0.15))];   // fpはキット素寸のセル単位
...
function worldSize(g) {   // 表示スケール適用後の実寸
  const s0 = cache.get(g.userData.item.model)?.size || { x: TILE0, z: TILE0 };
  const size = { x: s0.x * ROOM_SCALE, z: s0.z * ROOM_SCALE };
...
async function buildRoom(data, pack = false) {
  lastLayout = data;   // 生活プレビュー用に保持
  life.data = null;    // レイアウトが変わったら歩行グリッドを作り直す
  if (roomGroup) { scene.remove(roomGroup); roomGroup = null; }
  roomGroup = new THREE.Group();
  scene.add(roomGroup);
  detachGizmo();
  if ([...data.shell, ...data.items].some((i) => (i.level || 0) > 0)) FLOOR_H = (await getModel('wall')).size.y * ROOM_SCALE + FLOOR_T;   // 1階分＝壁の高さ＋床厚（実測×スケール）
  const jobs = [];
...
// ── 生活プレビュー: ken VRM が1日のスケジュールで部屋を生活する（lib/room-life.js と共用）──
const LIFE_ACTIONS = [
  { key: 'walk', label: '歩行', def: 'Catwalk_Walk_Forward.vrma' },
  { key: 'idle', label: '待機', def: 'Standing_Idle.vrma' },
  { key: 'sleep', label: '睡眠', def: '' },
  { key: 'sit', label: '座る', def: '' },
  { key: 'eat', label: '食事', def: '' },
  { key: 'cook', label: '料理', def: '' },
  { key: 'bath', label: '入浴', def: '' },
  { key: 'toilet', label: 'トイレ', def: '' },
];
const LIFE_WALK_SPEED = 1.1;   // m/s（実時間。時計の早送りとは独立＝見た目が破綻しない）
const life = {
  on: false, vrm: null, mixer: null, clips: new Map(), curAct: null, curAnimName: '',
  animMap: {}, clock: 6.4, speed: 480, data: null, entry: null,
  path: null, seg: 0, pos: new THREE.Vector3(), yF: 0, targetRy: null, onArrive: null, out: false,
};
let lastLayout = null;
const _clock3 = new THREE.Clock();
const _lifeHead = new THREE.Vector3();
...
async function loadLifeNpc() {
  ...
  life.vrm = gltf.userData.vrm;
  life.mixer = new THREE.AnimationMixer(life.vrm.scene);
  scene.add(life.vrm.scene);
}
async function lifeClip(name) {
  if (life.clips.has(name)) return life.clips.get(name);
  const res = await fetch('../vrma/' + encodeURIComponent(name));
  if (!res.ok) throw new Error('VRMAが見つかりません: ' + name);
  ...
  const clip = createVRMAnimationClip(ag.userData.vrmAnimations[0], life.vrm);
  stripRootMotion(clip);
  life.clips.set(name, clip);
  return clip;
}
async function playLifeAnim(name) {
  if (life.curAnimName === name) return;
  life.curAnimName = name;
  const prev = life.curAct;
  if (!name) { if (prev) prev.fadeOut(0.2); life.curAct = null; return; }
  try {
    const clip = await lifeClip(name);
    if (life.curAnimName !== name) return;   // 待っている間に切り替わった
    const act = life.mixer.clipAction(clip);
    act.reset().setLoop(THREE.LoopRepeat, Infinity).fadeIn(0.15).play();
    if (prev && prev !== act) prev.fadeOut(0.15);
    life.curAct = act;
...
function lifeGoTo(spot, onArrive) {
  ...
  life.path = path; life.seg = 0;
  life.targetRy = spot.ry ?? null;
...
function lifeArrived(entry, spot) {
  ...
  showLifeLabel(entry.label + (anim ? '' : '（アニメ未割当）'));
}
function setLifeEntry(entry, teleport) {
  life.entry = entry;
  $('life-action').textContent = `${fmtClock(life.clock)} — ${entry.label}`;
  const spots = life.data.spots;
  if (entry.action === 'out') {   // 玄関へ歩いて退場→外出中
    const door = spots.door;
    const leave = () => { life.out = true; life.vrm.scene.visible = false; showLifeLabel('外出中'); };
    if (!door) { leave(); return; }
    if (teleport) { lifePlaceAt(door, door.ry); leave(); return; }
    lifeGoTo(door, leave);
    playLifeAnim(lifeAnimFor('walk'));
    return;
  }
  if (life.out || entry.action === 'return') {   // 帰宅: 玄関に現れる
    life.out = false; life.vrm.scene.visible = true;
    const door = spots.door || { x: 0, z: 0, level: 0, ry: 0 };
    lifePlaceAt(door, door.ry + Math.PI);
    if (entry.action === 'return') { lifeArrived(entry, null); return; }
  }
  const spot = spots[entry.spot];
  if (!spot) { lifeArrived({ ...entry, label: entry.label + '（設備なし）' }, null); return; }
  if (teleport) { lifePlaceAt(spot, spot.ry); lifeArrived(entry, spot); return; }
  lifeGoTo(spot, () => lifeArrived(entry, spot));
  playLifeAnim(lifeAnimFor('walk'));
  showLifeLabel(entry.label + 'へ…');
}
async function toggleLife() {
  if (life.on) {
    life.on = false;
    if (life.vrm) life.vrm.scene.visible = false;
    hideLifeLabel();
    $('btn-life').textContent = '▶ プレビュー開始（NPCが生活する）';
    return;
  }
  if (!lastLayout) { setStatus('先に部屋を生成してください'); return; }
  await loadLifeNpc();
  if (!life.data) life.data = buildLifeData(lastLayout, { tile: TILE0, sizeOf: sizeOfItem });   // グリッドはセル単位（sizeOfも素寸）
  life.on = true;
  life.out = false;
  life.vrm.scene.visible = true;
  $('btn-life').textContent = '⏸ プレビュー停止';
  setLifeEntry(scheduleAt(life.clock), true);
}
function sizeOfItem(it) {
  if (it.unit) { const d = unitDefs.get(it.unit); return d ? { x: (d.fp?.[0] || 1) * TILE0, z: (d.fp?.[1] || 1) * TILE0 } : null; }
  const e = cache.get(it.model);
  return e ? { x: e.size.x, z: e.size.z } : null;
}
function updateLife(dt) {
  if (!life.on || !life.vrm) return;
  // ゲーム内時計
  const prev = life.clock;
  life.clock = (life.clock + dt * life.speed / 3600) % 24;
  if (Math.floor(prev * 60) !== Math.floor(life.clock * 60)) {
    $('life-clock').value = String(life.clock);
    $('life-clock-val').textContent = fmtClock(life.clock);
  }
  const entry = scheduleAt(life.clock);
  if (entry !== life.entry) setLifeEntry(entry, false);
  // 経路歩行（実時間・階段はyFを補間）
  if (life.path) {
    let move = LIFE_WALK_SPEED * dt / TILE;   // タイル/秒
    while (move > 0 && life.seg < life.path.length) {
      const wp = life.path[life.seg];
      const dx = wp.x * TILE - life.pos.x, dz = wp.z * TILE - life.pos.z;
      const dist = Math.hypot(dx, dz) / TILE;
      if (dist < 1e-4) { life.yF = wp.yF; life.seg++; continue; }
      const step = Math.min(move, dist);
      life.pos.x += dx * (step / dist);
      life.pos.z += dz * (step / dist);
      life.yF += (wp.yF - life.yF) * (step / dist);
      const ty = Math.atan2(dx, dz);
      let d = ty - life.vrm.scene.rotation.y;
      while (d > Math.PI) d -= Math.PI * 2; while (d < -Math.PI) d += Math.PI * 2;
      life.vrm.scene.rotation.y += d * Math.min(1, 10 * dt);
      move -= step;
      if (step >= dist - 1e-4) { life.yF = wp.yF; life.seg++; }
    }
    if (life.seg >= life.path.length) {
      life.path = null;
      const cb = life.onArrive; life.onArrive = null;
      if (cb) cb();
    }
  } else if (life.targetRy != null) {   // 到着後、スポットの向きへ
    let d = life.targetRy - life.vrm.scene.rotation.y;
    while (d > Math.PI) d -= Math.PI * 2; while (d < -Math.PI) d += Math.PI * 2;
  ...
  // 頭上ラベルを画面へ投影
  const el = $('life-label');
  if (el.style.display !== 'none' && life.vrm.scene.visible) {
    _lifeHead.set(life.pos.x, y + 1.75, life.pos.z).project(camera);
    if (_lifeHead.z < 1) {
      el.style.left = ((_lifeHead.x * 0.5 + 0.5) * window.innerWidth) + 'px';
      el.style.top = ((-_lifeHead.y * 0.5 + 0.5) * window.innerHeight) + 'px';
    }
  }
}
// アニメ割当UI: 行動ごとにVRMAを選ぶ → public/npc/furniture-anims.json（ゲームと共用予定）
async function buildLifeAnimUI() {
  let vrmas = [];
  try { vrmas = await (await fetch('../vrma/manifest.json')).json(); } catch { /* 開発サーバ以外 */ }
  try { life.animMap = await (await fetch('../npc/furniture-anims.json')).json() || {}; } catch { life.animMap = {}; }
  for (const a of LIFE_ACTIONS) if (life.animMap[a.key] == null && a.def) life.animMap[a.key] = a.def;
  const box = $('life-anims');
  for (const a of LIFE_ACTIONS) {
...
async function saveLifeAnims() {
  try {
    const r = await fetch('../api/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dir: 'npc', filename: 'furniture-anims.json', content: JSON.stringify(life.animMap, null, 1) }) });
    setStatus(r.ok ? '保存しました: npc/furniture-anims.json' : '保存失敗: ' + r.status);
...
async function init() {
  ...
  // 生活プレビュー
  $('btn-life').addEventListener('click', () => toggleLife().catch((e) => setStatus('プレビュー失敗: ' + e.message)));
  $('life-clock').addEventListener('input', () => {
    life.clock = parseFloat($('life-clock').value) || 0;
    $('life-clock-val').textContent = fmtClock(life.clock);
    if (life.on && life.data) setLifeEntry(scheduleAt(life.clock), true);   // シークは瞬間移動
  });
  $('life-speed').addEventListener('change', () => { life.speed = parseFloat($('life-speed').value) || 0; });
  $('btn-life-anims-save').addEventListener('click', saveLifeAnims);
  buildLifeAnimUI().catch(() => { /* manifest無し */ });
  ...
  TILE0 = Math.max(floor.size.x, floor.size.z) || 1;
  TILE = TILE0 * ROOM_SCALE;
  FLOOR_T = (floor.size.y || 0) * ROOM_SCALE;
  console.log('TILE =', TILE, 'FLOOR_T =', FLOOR_T);
  refreshLoadList();
  refreshUnitList();
  regenerate();
  renderer.setAnimationLoop(() => {
    const dt = Math.min(_clock3.getDelta(), 1 / 20);
    updateLife(dt);
